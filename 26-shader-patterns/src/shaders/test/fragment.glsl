...
float sharpPerlin(vec2 uv) {
    return step(0.8, sin(cnoise(uv * 10.0 + vTime * 0.3) * 20.0 + vTime));
}
...
void main()
{
    ...
    // Ex8
    // float strength = sudoRandom(vUv);

    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // Ex9
    // float coord1 = floor(vUv.x * 10.0) / 10.0;
    // float coord2 = floor(vUv.y * 10.0) / 10.0;
    // vec2 coord = vec2(coord1, coord2);

    // float strength = sudoRandom(coord);

    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // Ex10
    // float coord1 = floor(vUv.x * 10.0) / 10.0;
    // float coord2 = floor((vUv.y + vUv.x * 0.5) * 10.0) / 10.0;
    // vec2 coord = vec2(coord1, coord2);
    ...
    // Ex26
    // float angle = atan(vUv.x - 0.25, vUv.y - 0.25);
    // float strength = angle;
    ...
    // Ex36
    float strength = sharpPerlin(vUv);
